...
export default function Dashboard() {
  ...
  const fetchGoals = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        for (const goal of actives) {
          await fetchTasksAndTrackings(goal.id);
        }
      }
    } catch (err) {
      console.error(err);
  ...
  const fetchTasksAndTrackings = async (goalId: string) => {
    try {
      ...
      if (tasksRes.ok) {
        ...
        setGoalTasks(prev => ({ ...prev, [goalId]: tasksData }));
        ...
        setTrackings(prev => ({ ...prev, ...trackingsMap }));
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const handleTrackSubmit = async (taskId: string, goalId: string) => {
    let value = parseFloat(todayValues[taskId]);
    if (isNaN(value)) return;

    const task = (goalTasks[goalId] || []).find(t => t.id === taskId);
    if (!task) return;
    ...
    if (value > 0 && value > maxValueCanAdd) value = maxValueCanAdd;
    else if (value < 0 && value < minValueCanAdd) value = minValueCanAdd;
    ...
    try {
      const response = await apiFetch(`/api/tasks/${taskId}/trackings`, {
        method: 'POST',
        body: JSON.stringify({ value, trackDate: new Date().toISOString(), trackNote: '' })
      });
      if (response.ok) {
        const json = await response.json();
        setTrackings(prev => ({ ...prev, [taskId]: [json.data, ...(prev[taskId] || [])] }));
        setTodayValues(prev => ({ ...prev, [taskId]: '' }));
      }
    } catch (err) {
      console.error(err);
  ...
  const calculateOverallProgress = (goalId: string) => {
  ...
  const handleCompleteGoal = async () => {
    ...
    try {
      const response = await apiFetch(`/api/goals/${goalId}`, {
        ...
        body: JSON.stringify({
          ...targetGoal,
          startDate: targetGoal.start_date,
          endDate: targetGoal.end_date,
          status: 'completed'
        })
      });
      if (response.ok) {
  ...
  const DRAFTS_PER_PAGE = 8; // 4 cols × 2 rows max
  ...
  const pagedDrafts = filteredDrafts.slice((draftPage - 1) * DRAFTS_PER_PAGE, draftPage * DRAFTS_PER_PAGE);

  // Reset to page 1 when search changes
  useEffect(() => { setDraftPage(1); }, [searchTerm]);
      ...
      <section className="space-y-8">
        {activeGoals.length === 0 ? (
          ...
          activeGoals.map(goal => {
            ...
            const overallProgress = calculateOverallProgress(goal.id);
            const daysRemaining = differenceInDays(new Date(goal.end_date), new Date());
            const totalDays = Math.max(1, differenceInDays(new Date(goal.end_date), new Date(goal.start_date)));
            const daysPassed = totalDays - daysRemaining;
            const timeProgress = Math.min(100, Math.max(0, Math.round((daysPassed / totalDays) * 100)));

            return (
              <div key={goal.id} className="space-y-6 pb-8 border-b border-gray-100 last:border-0 last:pb-0">
                <div className="flex items-start justify-between">
                  ...
                  {overallProgress === 100 ? (
                    <button
                      onClick={() => { setTargetGoal(goal); setShowCompleteConfirm(true); }}
                    ...
                    <button
                      onClick={() => { setTargetGoal(goal); setShowEarlyEndConfirm(true); }}
                ...
                <div className="bg-white p-3 rounded-2xl shadow-sm border border-gray-200">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex-1 bg-gray-100 rounded-full h-4 overflow-hidden">
                      <div className="bg-indigo-600 h-full rounded-full transition-all duration-500" style={{ width: `${overallProgress}%` }} />
                    </div>
                    ...
                    return (
                      <div key={task.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-200 flex flex-col sm:flex-row sm:items-center gap-2">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          />
                          <button
                            onClick={() => handleTrackSubmit(task.id, goal.id)}
                ...
                <div className="flex items-center justify-center gap-2 mt-6">
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                    <button
                      ...
                      className={`w-9 h-9 rounded-lg text-sm font-medium transition-colors ${p === draftPage ? 'bg-gray-900 text-white' : 'border border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                    >
                      {p}
                    </button>
                  ))}
